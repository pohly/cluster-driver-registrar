...
configvar CSI_PROW_E2E_ALPHA_1_13 '\[Feature: \[Testpattern:.Dynamic.PV..block.volmode.\]' "alpha tests for Kubernetes 1.13" # Raw block was an alpha feature in 1.13.
...
start_cluster () {
    ...
    if [[ "${CSI_PROW_KUBERNETES_VERSION}" =~ ^release-|^latest$ ]]; then
        if ! ${csi_prow_kind_have_kubernetes}; then
            git_checkout https://github.com/kubernetes/kubernetes "$GOPATH/src/k8s.io/kubernetes" "${CSI_PROW_KUBERNETES_VERSION}" --depth=1 || die "checking out Kubernetes failed"
    ...
    while IFS= read -r line; do
        ...
        if echo "$line" | grep -q "^### [^ ]*:$"; then
            ...
            if [ "$testname" ]; then
                ...
                echo "    <failure>" >>"$out"
                echo -n "$testoutput" | sed -e 's/&/&amp;/g' -e 's/</\&lt;/g' -e 's/>/\&gt;/g' >>"$out"
                echo "    </failure>" >>"$out"
                echo "  </testcase>" >>"$out"
            fi
            ...
            if [ "$testname" ]; then
                # Test output. We must escape special characters and remove
                # escape sequences (no good representation in the simple XML
                # that we generate here). filter_junit.go would choke on them
                # during decoding, even when disabling strict parsing.
                echo "$line" | sed -e 's/&/&amp;/g' -e 's/</\&lt;/g' -e 's/>/\&gt;/g' -e 's/\x1B...//g'>>"$out"
